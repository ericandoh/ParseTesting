...
//        backImageView!.contentMode = UIViewContentMode.Center;
        ...
        if (imgBuffer == nil) { NSLog("imgBuffer is nil")
           ...
           if (imgBuffer!.getImagePostAt(viewCounter).myObj.objectId != nil) { NSLog("owner: \(self.imgBuffer!.owner)")
                if self.imgBuffer!.owner == "SUGGEST" || self.imgBuffer!.owner == "COLLECTION" { // for posts in explore, profile page
...
//            backImageView!.contentMode = UIViewContentMode.Center;
    ...
    func syncForUploadedPost(theirBuffer: CustomImageBuffer, selectedAt: Int) {
        NSLog("Syncing to home at \(selectedAt)")
        viewCounter = selectedAt;
        postCounter = 0;
        refreshNeeded = false;
        viewingComments = false;
        imgBuffer = theirBuffer;
        imgBuffer!.switchContext(theirBuffer.owner, nil, configureCellFunction: configureCurrent);
    }
